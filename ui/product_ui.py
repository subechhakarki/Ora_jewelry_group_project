...
def _project_root():
    return os.path.dirname(os.path.dirname(__file__))  # project root folder directory name
...
class ProductFormDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, product=None, on_saved=None):
        ...
        # Dill the stuff in the form when selected
...
class AdminProductManagementScreen:
    ...
    def build_ui(self):
        ...
        title = ctk.CTkLabel(title_left, text="🛍️ Admin — Product Management", **get_label_style("heading"))
        title.pack(anchor="w")
        ...
        search_btn = ctk.CTkButton(
            search_row, text="Search", width=110, command=self.search, **get_button_style("secondary")
        )
        search_btn.pack(side="left")


        clear_btn = ctk.CTkButton(
            search_row, text="Clear", width=90, command=self.refresh_products, **get_button_style("secondary")
        )
        ...
        preview_card.grid_rowconfigure(0, weight=0)
        ...
        self.btn_edit = ctk.CTkButton(
            actions, text="Edit", command=self.open_edit_dialog, state="disabled", **get_button_style("primary")
        )
        self.btn_edit.grid(row=0, column=0, sticky="ew", padx=(0, 8), pady=(0, 10))


        self.btn_stock = ctk.CTkButton(
            actions, text="Update Stock", command=self.open_stock_dialog, state="disabled", **get_button_style("secondary")
        )
        self.btn_stock.grid(row=0, column=1, sticky="ew", padx=(8, 0), pady=(0, 10))


        self.btn_delete = ctk.CTkButton(
            actions, text="Delete", command=self.delete_selected, state="disabled", **get_button_style("danger")
        )
        self.btn_delete.grid(row=1, column=0, sticky="ew", padx=(0, 8))


        self.btn_refresh = ctk.CTkButton(
            actions, text="Refresh", command=self.refresh_products, **get_button_style("secondary")
        )
    ...
    def _set_status(self, total: int, mode: str):
        if mode == "search":
            self.status_label.configure(text=f"Results: {total}")
        else:
    ...
    def _create_product_card(self, p: dict):
        ...
        if thumb_img is not None:
            ...
            thumb_label.configure(
                text="🖼️",
                font=(Fonts.FAMILY, Fonts.HEADING if hasattr(Fonts, "HEADING") else Fonts.NORMAL),
                text_color=Colors.TEXT_SECONDARY,
            )
        ...
        def on_leave(_):
            if self.selected_product and self.selected_product.get("product_id") == p.get("product_id"):
                card.configure(fg_color=Colors.BG_LIGHT)
            else:
                card.configure(fg_color=Colors.BG_WHITE)
    ...
    def _set_selected(self, product):
        ...
        if big_img is not None:
            ...
            self.preview_img_label.configure(image=None, text="🖼️ No image", text_color=Colors.TEXT_SECONDARY)
            self._sidebar_img_ref = None


        desc = (product.get("product_description") or "").strip()
    ...
    def delete_selected(self):
        ...
        os.remove(self.selected_product.get('product_image'))
...
class UserProductBrowseScreen:
    ...
    def add_to_cart(self, product):
        ...
        AddToCartDialog(self.parent, user_id=user_id, product=product, on_added=after_added)
    ...
    def build_ui(self):
        ...
        back_btn = ctk.CTkButton(
            top, text="← Back", command=self.go_back, width=110, **get_button_style("secondary")
        )
        ...
        search_btn = ctk.CTkButton(
            search_row, text="Search", command=self.search, width=120, **get_button_style("primary")
        )
        search_btn.pack(side="left", padx=(0, 10))


        clear_btn = ctk.CTkButton(
            search_row, text="Clear", command=self.load_products, width=90, **get_button_style("secondary")
        )
    ...
    def _build_product_card(self, parent, product):
        ...
        img_box = ctk.CTkFrame(card, fg_color="#F4F1FF", corner_radius=16, height=140)
        ...
        ctk_img = self._safe_ctk_image(img_src, size=(220, 140))
        if ctk_img:
            img_label.configure(image=ctk_img, text="")
            self._img_refs[f"card_{product.get('product_id', id(product))}"] = ctk_img
        else:
            img_label.configure(text="🖼️", font=(Fonts.FAMILY, 40), text_color=Colors.TEXT_SECONDARY)


        name = ctk.CTkLabel(
            card,
            text=product.get("product_name", "Unnamed"),
        ...
        price_text = f"Rs {product.get('product_price', 0)}"
        stock_text = f"Stock: {product.get('product_quantity', 0)}"
        meta = ctk.CTkLabel(
            card,
    ...
    def _safe_ctk_image(self, src: str, size=(200, 140)):
        """
        Loads a CTkImage from:
    ...
    def open_product_details(self, product):
        messagebox.showinfo(
            ...
            f"Price: Rs {product.get('product_price', 0)}\n"
